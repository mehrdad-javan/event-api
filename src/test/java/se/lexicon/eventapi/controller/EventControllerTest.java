...
class EventControllerTest {
    ...
    private EventParticipantRequestDTO participantRequestDTO;
    ...
    void setUp() {
        ...
        participantRequestDTO = new EventParticipantRequestDTO(
                "creator@test.com",
                "Event Creator"
        );
    }
    ...
    @Test
    @DisplayName("POST /api/v1/events/{eventId}/participants should return 201")
    void addParticipant_shouldSucceed() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/events/1/participants")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("GET /api/v1/events/{id} should return event with correct attendee count")
    void findById_shouldReturnCorrectAttendeeCount() throws Exception {
        event.getParticipants().add(creator); // Add 1 participant
        when(eventRepository.findById(1L)).thenReturn(Optional.of(event));

        mockMvc.perform(get("/api/v1/events/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.attendees").value(1));
    }
}
